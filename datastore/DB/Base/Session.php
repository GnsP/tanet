...
abstract class Session implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\DB\\Map\\SessionTableMap';
    ...
    /**
     * Initializes internal state of DB\Base\Session object.
     */
     ...
     * @return $this|\DB\Session The current object (for fluent API support)
     */
    ...
    public function setUid($v)
    {
        ...
        if ($this->uid !== $v) {
            $this->uid = $v;
            $this->modifiedColumns[SessionTableMap::COL_UID] = true;
        }
    ...
    public function setExpires($v)
    {
        ...
        if ($this->expires !== null || $dt !== null) {
            if ($this->expires === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->expires->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[SessionTableMap::COL_EXPIRES] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionTableMap::translateFieldName('Uid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->uid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionTableMap::translateFieldName('Expires', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\DB\\Session'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SessionTableMap::COL_UID)) {
            $modifiedColumns[':p' . $index++]  = 'uid';
        }
        if ($this->isColumnModified(SessionTableMap::COL_EXPIRES)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\DB\Session The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionTableMap::COL_UID)) {
            $criteria->add(SessionTableMap::COL_UID, $this->uid);
        }
        if ($this->isColumnModified(SessionTableMap::COL_EXPIRES)) {
            $criteria->add(SessionTableMap::COL_EXPIRES, $this->expires);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \DB\Session (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \DB\Session Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aUser === null && ($this->uid != 0)) {
            $this->aUser = ChildUserQuery::create()->findPk($this->uid, $con);
