 ...
 * @method     ChildUserQuery orderByPremissions($order = Criteria::ASC) Order by the premissions column
 *
 ...
 * @method     ChildUserQuery groupByPremissions() Group by the premissions column
 *
 ...
 *
 * @method     ChildUserQuery leftJoinTeacher($relationAlias = null) Adds a LEFT JOIN clause to the query using the Teacher relation
 * @method     ChildUserQuery rightJoinTeacher($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Teacher relation
 * @method     ChildUserQuery innerJoinTeacher($relationAlias = null) Adds a INNER JOIN clause to the query using the Teacher relation
 *
 * @method     ChildUserQuery joinWithTeacher($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Teacher relation
 *
 * @method     ChildUserQuery leftJoinWithTeacher() Adds a LEFT JOIN clause and with to the query using the Teacher relation
 * @method     ChildUserQuery rightJoinWithTeacher() Adds a RIGHT JOIN clause and with to the query using the Teacher relation
 * @method     ChildUserQuery innerJoinWithTeacher() Adds a INNER JOIN clause and with to the query using the Teacher relation
 *
 ...
 *
 * @method     \DB\TeacherQuery|\DB\SessionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildUser findOneByPremissions(string $premissions) Return the first ChildUser filtered by the premissions column *
 ...
 * @method     ChildUser requireOneByPremissions(string $premissions) Return the first ChildUser filtered by the premissions column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUser[]|ObjectCollection findByPremissions(string $premissions) Return ChildUser objects filtered by the premissions column
...
abstract class UserQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \DB\Base\UserQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\DB\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, password_hash, salt, premissions FROM user WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the premissions column
     *
     * Example usage:
     * <code>
     * $query->filterByPremissions('fooValue');   // WHERE premissions = 'fooValue'
     * $query->filterByPremissions('%fooValue%', Criteria::LIKE); // WHERE premissions LIKE '%fooValue%'
     * </code>
     *
     * @param     string $premissions The value to use as filter.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function filterByPremissions($premissions = null, $comparison = null)
    {
        if (null === $comparison) {
        ...
        return $this->addUsingAlias(UserTableMap::COL_PREMISSIONS, $premissions, $comparison);
    }

    /**
     * Filter the query by a related \DB\Teacher object
     *
     * @param \DB\Teacher|ObjectCollection $teacher the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    {
        if ($teacher instanceof \DB\Teacher) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $teacher->getId(), $comparison);
            ...
            throw new PropelException('filterByTeacher() only accepts arguments of type \DB\Teacher or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \DB\TeacherQuery A secondary query class using the current class as primary query
     */
    public function useTeacherQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinTeacher($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Teacher', '\DB\TeacherQuery');
    }

    /**
     * Filter the query by a related \DB\Session object
     *
     * @param \DB\Session|ObjectCollection $session the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildUserQuery The current query, for fluid interface
     */
    ...
    {
        if ($session instanceof \DB\Session) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $session->getUid(), $comparison);
            ...
            throw new PropelException('filterBySession() only accepts arguments of type \DB\Session or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \DB\SessionQuery A secondary query class using the current class as primary query
     */
    public function useSessionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinSession($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Session', '\DB\SessionQuery');
    }
