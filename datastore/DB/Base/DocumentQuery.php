...
use \Exception;
use \PDO;
use DB\Document as ChildDocument;
use DB\DocumentQuery as ChildDocumentQuery;
use DB\Map\DocumentTableMap;
 ...
 * @method     ChildDocumentQuery orderByName($order = Criteria::ASC) Order by the name column
 *
 * @method     ChildDocumentQuery groupById() Group by the id column
 * @method     ChildDocumentQuery groupByName() Group by the name column
 *
 ...
 * @method     ChildDocument findOneByName(string $name) Return the first ChildDocument filtered by the name column *
 ...
 * @method     ChildDocument requireOneByName(string $name) Return the first ChildDocument filtered by the name column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildDocument[]|ObjectCollection findByName(string $name) Return ChildDocument objects filtered by the name column
...
abstract class DocumentQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \DB\Base\DocumentQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\DB\\Document', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = DocumentTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT id, name FROM documents WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            DocumentTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(DocumentTableMap::COL_ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(DocumentTableMap::COL_ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(DocumentTableMap::COL_NAME, $name, $comparison);
    }
    ...
    {
        if ($document) {
            $this->addUsingAlias(DocumentTableMap::COL_ID, $document->getId(), Criteria::NOT_EQUAL);
        }
